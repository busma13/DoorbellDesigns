...
$client = new SquareClient([
    ...
    'environment' => 'Environment::SANDBOX', // update for production
]);
...
if (isset($_POST['submit'])) {
    ...
    if (empty($first) || empty($last) || empty($email) || empty($address_line) || empty($city) || empty($state) || empty($zip) || empty($products)) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $post_data = array($id, $first, $last, $tel, $email, $address_line, $city, $state, $zip, $products);
            // createPaymentLink($client, $post_data);
