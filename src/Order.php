...
use stdClass, PDO, PDOException, App;
...
$customer->setAddress_Line($_POST['address-line']);
$customer->setCity($_POST['city']);
$customer->setState($_POST['state']);
$customer->setZip($_POST['zip']);

$order->setCustomer($customer);
$order->setProducts(json_decode($_POST['cart-list-input'], false));

//check that order has all required information
if ($order->orderHasEmptyFields()) {
    header("Location: ../checkout.php?order=empty#message" . $order->getProducts() . $order->getCustomer()->toString());
    exit();
}
// Check for valid email address
if (!filter_var($order->getCustomer()->getEmail(), FILTER_VALIDATE_EMAIL)) {
    header("Location: ../checkout.php?order=email#message");
    exit();
}

// Create a payment link.  This includes a Square order object.
...
for ($i = 0; $i < count($order->getProducts()); $i++) {
    $priceCents = 0;
    $obj = $order->getProducts()[$i];
    // echo var_dump($obj);
    // echo '<br>';
    // echo $obj->itemName;
    // echo '<br>';
    ...
    $itemNameWithOptions = Utils::setItemName($obj);
