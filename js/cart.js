...
function addToCart(event) {
  ...
  product = ls
    .get("productList")
    .find((product) => product.itemName === productName)
...
function toast() {
  ...
  toastBody.innerHTML =
    '<button type="button" class="close-toast aria-label="Close"><span aria-hidden="true">&times;</span></button> <span class="lnr lnr-cart"></span><span> Added To Cart!</span>'
...
function updateSessionStorageCartContents(qtyToAdd, product) {
  ...
  if (cartContents) {
    ...
    if (!cartContents[key]) {
      //no product with this name & options in cart
      ...
      //product with this name & options is in the cart
      cartContents[key].priceInCart = calculateLineItemTotal(
        qtyToAdd,
        cartContents[key]
      )
...
function calculateLineItemTotal(qtyToAdd, cartItem) {
  ...
  if (priceArray.length === 2) {
    if (newQty % 2 === 0) {
      ...
      priceTotal = (
        Number(priceArray[0]) +
        Number(priceArray[1]) * ((newQty - 1) / 2)
      ).toFixed(2)
    }
  } else {
