...
document.getElementById("upload_widget").addEventListener(
  ...
  function () {
    ...
    const picNumber =
      Number(
        addImageSelect.options[addImageSelect.selectedIndex].dataset.numpics
      ) + 1
...
async function updateImageUrl(picInfo, numberOfPics, productId) {
  ...
  try {
    const response = await fetch("./src/AddImage.php", {
      method: "POST",
      mode: "same-origin",
      cache: "no-cache",
    ...
    const responseMessageAddImage = document.querySelector(
      ".responseMessageAddImage"
    )

    if (data === "success") {
      responseMessageAddImage.textContent = "Image successfully added."
    } else if (data === "add-image-failed") {
      responseMessageAddImage.textContent =
        "Error adding image. Please try again."
    } else if (data === "duplicate-file-name") {
      responseMessageAddImage.textContent =
        "An image with that name already exists. Please rename the file to an unused name and add the image again."
    } else {
...
function sendValuesToServer(cell) {
  ...
  if (cell.parentNode.id.length > 4) {
    //cell is from show table
...
async function updateSchedule(obj) {
  try {
    const response = await fetch("./src/EditShowSchedule.php", {
      method: "POST",
      mode: "same-origin",
      cache: "no-cache",
      credentials: "same-origin",
...
async function updateProduct(obj) {
  try {
    const response = await fetch("./src/EditProduct.php", {
      method: "POST",
      mode: "same-origin",
      cache: "no-cache",
      credentials: "same-origin",
    ...
    const responseMessageProduct = document.querySelector(
      ".responseMessageProduct"
    )

    if (data === "success") {
      responseMessageProduct.textContent = "Product successfully edited."
    } else if (data === "image-resize error") {
      responseMessageProduct.textContent =
        "Failed to resize images. Please try again."
    } else {
...
async function deleteShow(event) {
  ...
  try {
    const response = await fetch("./src/DeleteShow.php", {
      method: "POST",
      mode: "same-origin",
      cache: "no-cache",
      credentials: "same-origin",
    ...
    if (data === "success") {
      ...
      responseMessageShow.textContent =
...
async function deleteProduct(event) {
  ...
  try {
    const response = await fetch("./src/DeleteProduct.php", {
      method: "POST",
      mode: "same-origin",
      cache: "no-cache",
      credentials: "same-origin",
